...
import { LargeSection, XLSection } from '@/components/layouts/Pages';
import {
  BlogPostMetadata,
...
export const getStaticProps = async (): Promise<{
  ...
  return {
    props: {
      stars: indexStaticProps.props.stars,
      status: indexStaticProps.props.status,
...
const About = ({ posts }: AboutPageStaticProps) => {
  return (
    <>
      <XLSection>
        <div className="mt-24 grid w-full grid-cols-1 items-center gap-8 rounded-md border border-neutral-900 bg-neutral-1000 px-6 py-8 sm:p-12 lg:grid-cols-2">
          <div className="text-base leading-loose">
            ...
            <p className="mt-4 text-neutral-500">
              ...
              computers. We projected magical capabilities onto them right away,
              yet for the next 70 years, they were to remain, at a fundamental
              level, tedious to work with. We had to adapt to them, not the
              other way around.
            </p>
            <p className="mt-4 text-neutral-500">
              This is changing, and it is happening now. Generative AI will
              completely transform the way we interact with computers, unlocking
              the magic potential we have been dreaming of for decades. At
              Markprompt, we are building the infrastructure that enables this
              tectonic shift to unfold within businesses, starting at the nerve
              center:{' '}
              <span className="font-medium text-neutral-300">
                customer support
              </span>
      ...
      <LargeSection>
        ...
        <div className="mt-12 flex flex-row items-center gap-2">
          ...
          <span className="text-base font-medium leading-[1.6] text-neutral-300">
            Combinator
          </span>
        </div>
...
const AboutWithLayout: FC<InferGetStaticPropsType<typeof getStaticProps>> = ({
  posts,
  stars,
  status,
}) => {
  return (
    <LandingLayout
      pageTitle="About"
      heading="Building the AI infrastructure for customer experience"
      subheading="Generative AI is changing every aspect of running a business, and it is happening now. Markprompt is born out of this revolution."
      huge
      stars={stars}
      status={status}
    >
